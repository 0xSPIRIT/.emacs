...
;; Ido. [Not using ido at the moment]
...
(setq program "d:/dev/alaska/bin/alaska.exe")
(setq fill-column 80)
...
(defun go-cast (&optional arg)
  (interactive "sCast to type: ")
  (if (use-region-p)
      (progn
	(let ((p1 (region-beginning))
	      (p2 (region-end)))
	  (setq go-cast-type "float64")
	  (if (equal "" arg)
	      ()
	    (setq go-cast-type arg))
	  (goto-char p2)
	  (insert ")")
	  (goto-char p1)
	  (insert (concat go-cast-type "("))
	  ))))
...
(global-set-key [C-f2] 'company-mode)
...
(custom-set-variables
 ...
 '(menu-bar-mode nil)
 '(mini-frame-completions-show-parameters '((height . 0.25) (width . 1.0) (left . 0.5)))
 '(mini-frame-ignore-commands
   '(quick-calc eval-expression "edebug-eval-expression" debugger-eval-expression))
 '(mini-frame-mode t)
...
(custom-set-faces
 ...
 '(ivy-current-match ((t (:extend t :background "gray15" :foreground "white" :box (:line-width (2 . 2) :color "grey70")))))
 '(ivy-minibuffer-match-face-2 ((t (:background "#222222" :weight bold))))
 '(ivy-minibuffer-match-highlight ((t (:inherit highlight :slant italic))))
 '(treemacs-fringe-indicator-face ((t nil))))
